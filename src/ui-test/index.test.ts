...
describe("UI Tests", () => {
  ...
  describe("Hatenablogger: Post or Update Command", () => {
    ...
    it("successfully uploads a image", async () => {
      ...
      await dialog.selectPath(`${process.cwd()}/src/ui-test/fixture/screenshot.png`);
      ...
      expect(await notification.getMessage()).to.be("Successfully image uploaded!");
      expect(await notification.getType()).equals(NotificationType.Info);
...
const setConfiguration = async ({
  ...
  hatenaId &&
    (await (await settingsEditor.findSetting("Hatena ID", "Hatenablogger")).setValue(hatenaId));
